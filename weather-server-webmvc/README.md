...
This sample project demonstrates how to create an MCP server using the Spring AI MCP Server Boot Starter with WebMVC
...
```bash
java -jar target/weather-server-webmvc-0.0.1-SNAPSHOT.jar
```

Run with sse mode for STREAMABLE protocol:

```bash
java -Dspring.ai.mcp.server.protocol=STREAMABLE -jar target/weather-server-webmvc-0.0.1-SNAPSHOT.jar
```
...
```bash
java -Dspring.ai.mcp.server.stdio=true -Dspring.main.web-application-type=none -jar target/weather-server-webmvc-0.0.1-SNAPSHOT.jar
```
